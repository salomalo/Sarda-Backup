...
class WF_ProdImpExpCsv_Product_Import extends WP_Importer {
    ...
    var $product_to_be_deleted_array = array();
...
// Remainder
    ...
    public function import_options() {
        ...
        if (( $handle = @fopen($file, "r") ) !== FALSE) {
            ...
            $header = ($this->decentFgetcsv) ? fgetcsv($handle, 0, $this->delimiter, '"', '"') : fgetcsv($handle, 0, $this->delimiter, '"');
    ...
    public function process_product($post, $new_product_status = null) {        
        ...
        if (isset($post['parent_sku']) && $post['parent_sku'] !== '' && $post['parent_sku'] !== null) {
            $prod_id = $piep_helper_object->xa_wc_get_product_id_by_sku($post['parent_sku']);
            if ($this->delete_products == 1) {
        ...
        if (!$merging) {
          ...
          $existing_product= $this->product_exists($processing_product_title, $processing_product_sku, $post['post_name']);
            ...
            foreach ($post['postmeta'] as $meta) {
                ...
                if ($this->prod_use_chidren_sku && '_children' == $key && 'grouped' == $product_type && !empty($meta['value'][0])) {
                    ...
                    foreach ($meta['value'] as $children_sku_key => $children_sku) {
                        $children_id[] = $piep_helper_object->xa_wc_get_product_id_by_sku($children_sku);
                    }
                    $meta['value'] = $children_id;
                }
                if ($key) {
                    update_post_meta($post_id, ( ( stristr($key, 'attribute') != false ) ? strtolower($key) : $key), maybe_unserialize($meta['value']));
                }
        ...
        if (!empty($post['images']) && is_array($post['images'])) {
            ...
            if ($merging) {
                ...
                foreach ($post['images'] as $image) {
                    $i = isset($image_basenames) ? count($image_basenames) : 0;
