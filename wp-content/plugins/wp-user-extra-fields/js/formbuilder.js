...
this["Formbuilder"]["templates"]["edit/text"]  = function(obj)
{
	...
	if(woocommerce_is_active)
	{
		__p +='.<br/><strong>NOTE:</strong> if the <strong>Password</strong> option is selected, the field will be displayed only in <strong>Register form</strong> (Checkout and Register page) and <strong>My Account page</strong> (according to visibility options) and will not be beformed any strong/weak security check';
	}
	__p +=':</p>';
...
this["Formbuilder"]["templates"]["edit/checkboxes"] = function(obj) 
{
	...
	with (obj) 
	{
		...
		__p += '<br/><br/><input type=\'checkbox\' data-rv-checked=\'model.editable_only_by_admin\' />\n Editable <strong>ONLY</strong> by Admin (User can see the field on hiss profile page but cannot edit it.';
		if(woocommerce_is_active)
		...
		__p += '<br/><br/><input type=\'checkbox\' data-rv-checked=\'model.invisible\' />\n Invisible: the field will not be visible in any page. You must use shortcodes to display.</input><br/><br/>';
		...
		if(woocommerce_is_active)
		{
			...
			__p += '<h4>Row size:</h4>'; 
			__p += '<p><strong>NOTE:</strong> this setting is ignored in case of <i>State & Country</i> field type.</p>'; 
			...
			//__p += '<br/><br/><input type=\'checkbox\' data-rv-checked=\'model.woocommerce_save_on_checkout_as_order_field\' />\n Save as order extra field\n</label>';
		}
	}
	return __p;
};
...
{
	//Modificato: aggiunto obj.rf.attributes.cid
	//__p += '<br/><label>\n  <input type=\'checkbox\' data-rv-checked=\'model.woocommerce_checkout_re_upload\' />\n Let customer re-upload a file on Checkout page (this will overwrite the previous uploaded file).<br/><strong>NOTE:</strong> Field will not be saved if guest checkout is allowed</label>';
...
__p += '<label>Limit size (MB)</label><br/><input type=\'number\' data-rv-input=\'model.file_size\' /><br/>Leave empty for no limits.';
...
'\' />\n    Include blank (by the default this will be the one selected)\n  </label>\n';
