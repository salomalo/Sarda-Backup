...
class OrdersImporter extends Importer {
    ...
    public function afterPostImport() {
        ...
        if (!empty($this->getArticleData('ID')) and $new_status !== $old_status && empty($this->getImport()->options['do_not_send_order_notifications'])) {
        ...
        if (empty($this->getArticleData('ID')) && empty($this->getImport()->options['do_not_send_order_notifications'])) {
            /** @var WC_Order $order */
