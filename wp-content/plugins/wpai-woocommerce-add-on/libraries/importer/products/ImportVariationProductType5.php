...
//        $this->setProperty('catalog_visibility', wc_clean( $this->getValue('product_visibility') ));
    ...
    protected function generateSKU() {
        // Unique SKU
        $sku = get_post_meta($this->getProduct()->get_id(), '_sku', true);
        ...
        if ($newSKU == '' && !$this->getImport()->options['disable_auto_sku_generation']) {
            if ($this->isNewVariation() || $this->getImportService()->isUpdateCustomField('_sku')) {
                $newSKU = substr(md5($this->getProduct()->get_title()), 0, 12);
            }
        }
        if ($newSKU != '' && $newSKU !== $sku) {
            if (!$this->getImport()->options['disable_sku_matching']
                ...
                $this->getLogger() && call_user_func($this->getLogger(), sprintf(__('- <b>WARNING</b>: Product SKU must be unique.', \PMWI_Plugin::TEXT_DOMAIN)));
            }
        }
        return $newSKU;
    }
    ...
    public function importCustomFields() {
        ...
        foreach (get_post_meta($this->getProduct()->get_id(), '') as $cur_meta_key => $cur_meta_val) {
    ...
    public function prepareAttributesProperties() {
        ...
        foreach ($attributes as $i => $attribute) {
            ...
            if ($attribute->is_taxonomy()) {
                ...
                $parsedAttributes[$attribute->get_name()] = $attribute->get_options();
            }
            // Add attribute to array, but don't set values.
            $parentAttributes[$attribute->get_taxonomy()] = array(
