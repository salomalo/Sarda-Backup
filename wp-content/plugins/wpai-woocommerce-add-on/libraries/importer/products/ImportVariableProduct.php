...
class ImportVariableProduct extends ImportProduct {
        ...
        try {
            ...
            if ($this->getImport()->options['link_all_variations'] && $this->getImportService()->isUpdateDataAllowed('is_update_attributes', $this->isNewProduct())) {
                ...
                if ('xml' == $this->getImport()->options['matching_parent']) {
                    ...
                    if (empty($this->getImport()->options['variable_sku']) && empty($this->getImport()->options['disable_auto_sku_generation'])) {
    ...
    protected function initParentProduct() {
        ...
        if ( "manual" != $this->getImport()->options['duplicate_matching'] || $this->isNewProduct() ) {
            ...
            if ( ! empty($this->getValue('single_product_parent_ID')) ){
                $postRecord = $this->wpdb->get_row($this->wpdb->prepare("SELECT * FROM " . $this->wpdb->prefix . "pmxi_posts WHERE `import_id` = %d AND `product_key` = %s ORDER BY post_id ASC", $this->getImport()->id, $this->getValue('single_product_parent_ID')));
            ...
            if (!empty($this->getArticleData('post_parent'))) {
                $parent_id = $this->getArticleData('post_parent');
            }
        }
    ...
    protected function setVariationStockProperties($variation) {
        ...
        if ($this->isFirstRowVariation()) {
            ...
            if ($this->getImportService()->isUpdateTaxonomy('product_shipping_class', $this->isNewProduct())) {
                ...
                $this->getImportService()->getTaxonomiesService()->associateTerms( $variation->get_id(), array($parent->get_shipping_class_id()), 'product_shipping_class' );
            }
        }
    }
