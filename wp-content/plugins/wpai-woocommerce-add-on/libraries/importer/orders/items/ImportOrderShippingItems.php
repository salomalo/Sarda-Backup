...
class ImportOrderShippingItems extends ImportOrderItemsBase {
    ...
    public function import() {
        ...
        if (empty($this->getArticleData('ID')) or $this->getImport()->options['update_all_data'] == 'yes' or $this->getImport()->options['is_update_shipping']) {
    ...
    protected function _import_shipping_items() {
        if (!empty($this->getValue('shipping'))) {
            $total_shipping = 0;

            foreach ($this->getValue('shipping') as $shippingIndex => $shipping) {
                ...
                if ($method) {
                    ...
                    if ($shipping_item->isEmpty()) {
                        ...
                        if (!empty($this->getArticleData('ID'))) {
