...
class ImportOrderFeeItems extends ImportOrderItemsBase {
    ...
    public function import() {
        if (empty($this->getArticleData('ID')) or $this->getImport()->options['update_all_data'] == 'yes' or $this->getImport()->options['is_update_fees']) {
    ...
    protected function _import_fee_items() {
        if (!empty($this->getValue('fees'))) {
            foreach ($this->getValue('fees') as $feeIndex => $fee) {
                if (empty($fee['name'])) {
                    continue;
                }
