...
class ImportOrderNotes extends ImportOrderBase {
    ...
    public function import() {
        if (empty($this->getArticleData('ID')) or $this->getImport()->options['update_all_data'] == 'yes' or $this->getImport()->options['is_update_notes']) {
    ...
    protected function _import_order_notes() {
        if (!empty($this->getValue('notes'))) {
            $notes_count = 0;
            foreach ($this->getValue('notes') as $noteIndex => $note) {
