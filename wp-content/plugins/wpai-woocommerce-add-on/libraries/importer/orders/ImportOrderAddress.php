...
class ImportOrderAddress extends ImportOrderBase {
    ...
    public function importBilling() {
        ...
        if (empty($this->getArticleData('ID')) or $this->getImport()->options['update_all_data'] == 'yes' or $this->getImport()->options['is_update_billing_details']) {
    ...
    public function importShipping() {
        ...
        if (empty($this->getArticleData('ID')) or $this->getImport()->options['update_all_data'] == 'yes' or $this->getImport()->options['is_update_shipping_details']) {
            switch ($this->getImport()->options['pmwi_order']['shipping_source']) {
                ...
                default:
                    foreach ($this->billing_fields as $billing_field) {
                        ...
                        $shipping_value = '';
                        if (!empty($this->getValue($shipping_field))) {
