...
class XmlImportWooPriceService extends XmlImportWooServiceBase {
     ...
     */
    public function adjustPrice($price, $field) {
        switch ($field) {
            case 'variable_regular_price':
            case 'regular_price':
                if (!empty($this->getImport()->options['single_product_regular_price_adjust'])) {
                    switch ($this->getImport()->options['single_product_regular_price_adjust_type']) {
                        case '%':
                            $price = ($price / 100) * $this->getImport()->options['single_product_regular_price_adjust'];
                            break;
                        case '$':
                            $price += (double) $this->getImport()->options['single_product_regular_price_adjust'];
                            break;
                    }
            ...
            case 'sale_price':
                if (!empty($this->getImport()->options['single_product_sale_price_adjust'])) {
                    switch ($this->getImport()->options['single_product_sale_price_adjust_type']) {
                        case '%':
                            $price = ($price / 100) * $this->getImport()->options['single_product_sale_price_adjust'];
                            break;
                        case '$':
                            $price += (double) $this->getImport()->options['single_product_sale_price_adjust'];
                            break;
                    }
                    $price = ((double) $price > 0) ? number_format((double) $price, 2, '.', '') : 0;
                }
