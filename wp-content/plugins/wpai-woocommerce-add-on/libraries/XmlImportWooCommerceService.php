...
final class XmlImportWooCommerceService {
    ...
    protected function __construct() {
        try {
            ...
            if ( ! $this->import->isEmpty() ) {
                ...
                throw new Exception(__('Can\'t resolve Import instance.', PMWI_Plugin::TEXT_DOMAIN));
            }
        }
        catch(\Exception $e) {
    ...
    public function setImport() {
        ...
        if ($importID && (empty($this->import) || $this->import->id != $importID)) {
            ...
            if (isset($parentAttributes[$tx_name])) {
                ...
                if ($parentAttributes[$tx_name]['is_taxonomy']) {
                    ...
                    foreach ($terms as $slug) {
                        $term = get_term_by('slug', urldecode($slug), $tx_name);
    ...
    public function maybeMakeProductSimple($product, $variationIDs) {
        ...
        if ($this->isUpdateDataAllowed('is_update_product_type', $isNewProduct) && $this->getImport()->options['make_simple_product']) {
            ...
            if ($price != "") {
                XmlImportWooCommerceService::getInstance()->pushMeta($product->get_id(), '_regular_price', $price, $isNewProduct);
            }
            elseif ($regularPrice != "") {
                XmlImportWooCommerceService::getInstance()->pushMeta($product->get_id(), '_price', $regularPrice, $isNewProduct);
            }
            ...
            if (empty($variationIDs)) {
                ...
                if (!$product->managing_stock()) {
                    // If manage stock disable set stock status to instock.
                    $simpleProduct->set_stock_status('instock');
                }
                $simpleProduct->save();
            }
