...
"use strict";
	...
	if ( $('#prdctfltr_save_default').length > 0 ) {
		$('.woocommerce_page_wc-settings .woocommerce.wrap p.submit:last').before('<p class="pf-submit" style="display:none;"><a href="#" id="prdctfltr_save_bottom" class="button-primary">' + $('#prdctfltr_save').text() + '</a> <a href="#" id="save" class="button-primary">' + $('#prdctfltr_save_default').text() + '</a></p>');
		...
		$('#save').on( 'click', function() {
			$('#prdctfltr_save_default').trigger('click');
			return false;
		});
	}
	...
	$(document).on('change', '.prdctfltr_adv_select', function() {
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				...
				curr_el.find('select').trigger('change');
			} else { 
	...
	$(document).on('click', '.prdctfltr_or_add', function() {
		...
		var curr_data = {
			...
			curr_tax: curr.attr('data-taxonomy'),
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				curr.prev().prepend('<span class="prdctfltr_override"><input type="checkbox" class="pf_override_checkbox" /><span class="slug">'+curr.find('.prdctfltr_or_select').val()+'</span><span class="preset">'+curr.find('.prdctfltr_filter_presets').val()+'</span> <a href="#" class="prdctfltr_or_remove"></a><span class="clearfix"></span></span>');
...
/*		if ( confirm(prdctfltr.localization.remove_override) === false ) {
	...
	var saving = false;
	$(document).on('click', '#prdctfltr_save, #prdctfltr_save_default', function() {

		if ( $(this).is('#prdctfltr_save_default') ) {
		...
		if ( !clicked.is('#prdctfltr_save_default') ) {
	...
	$(document).on('click', '.prdctfltr_or_remove_selected', function() {
		var curr = $(this).closest('p').prev();

		curr.find('input.pf_override_checkbox:checked').each( function() {
			$(this).parent().find('.prdctfltr_or_remove').trigger('click');
		});
	...
	$(document).on('click', '.prdctfltr_or_remove_all', function() {
		...
		curr.find('.prdctfltr_or_remove').each( function() { $(this).trigger('click'); } );
...
/*	$(document).on('change', 'select#prdctfltr_filter_presets', function() {
	...
	function check_ajax() {
		...
		if ( current.is(':checked') ) {
			...
			parent.find('#wc_settings_prdctfltr_ajax_templates').closest('tr').hide();
		}
	}
	check_ajax();
