...
function aspwc_get_posts_matching_conditions() {
	...
	foreach ( $posts as $post ) {
		...
		if ( wpc_match_conditions( $condition_groups, array( 'context' => 'aspwc', 'package' => $shipping_packages[0] ) ) == true ) {
...
function aspwc_split_cart_shipping_packages( $packages ) {
	...
	$default_package = $origin_packages[0];
...
function aspwc_exclude_shipping_rates( $rates, $package ) {
	...
	$excluded_rates = get_post_meta( $package['id'], '_exclude_shipping', true );

	foreach ( $rates as $k => $rate ) {
		if ( array_intersect( array( $rate->id, $rate->method_id ), $excluded_rates ) ) {
			unset( $rates[ $k ] );
		}
	}
