...
<?php //wp_die(var_dump($availableClones)); ?>
        <?php foreach ($availableClones as $name => $data):?>
            <div id="<?php echo $data["directoryName"]; ?>" class="wpstg-clone">
                ...
                <?php echo apply_filters("wpstg_after_stage_buttons", $html = 'test', $name, $data)?>
                <div class="wpstg-staging-info">                   
                    ...
                    echo $prefix;
                    echo '</br>';
                    $datetime = isset ($data['datetime']) ? __("Updated: <span>" . date("D, d M Y H:i:s T",$data['datetime']) , "wp-staging") . '</span> ' : '&nbsp;&nbsp;&nbsp;';
                    echo $datetime;
                    ?>
                </div>
            </div>
