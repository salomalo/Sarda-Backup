...
class SearchReplace extends \WPStaging\Backend\Modules\Jobs\JobExecutable {
   ...
   private function searchReplace( $table, $page, $args ) {
      ...
      $args['search_for'] = array(
          str_replace( '/', '\/', $this->get_url_without_scheme( $this->options->existingClones[strtolower( $this->options->current )]['url'] ) ),
          $this->get_url_without_scheme( $this->options->existingClones[strtolower( $this->options->current )]['url'] ),
      ...
      $args['replace_with'] = array(
          str_replace( '/', '\/', $helper->get_home_url_without_scheme()  ),
          $helper->get_home_url_without_scheme(),
          rtrim( ABSPATH, '/' ),
      );

      $this->debugLog( 'Search: ' . $this->get_url_without_scheme( $this->options->existingClones[strtolower( $this->options->current )]['url'] ) . ' Replace: ' . $this->get_url_without_scheme( $helper->get_home_url() ) );
