<?php
/*
Plugin Name: WP Related Items PRO
...
$this_version = '1.2.5';
$this_plugin_name = 'WP Related Items PRO';
...
/** 
 * This plugin contains the following packages, find below the corresponding copyright and license information for them: 
 ...
 * @license    You are free to use the example plugin code in any of your projects without concern for the copyright. (Request 1221 - 2016 Mar 25, 00:52 - Todd Lahman�(Todd Lahman))	
 * @link       https://www.toddlahman.com/shop/api-code-integration/
 * 
 * subdir: am
 * 
 */
...
class WRI {
	...
	function __construct() {
		...
		if ( 1 == $wri_general_settings['enable_plugin'] && is_plugin_active('yet-another-related-posts-plugin/yarpp.php')) {  //if WRI plugin and yarpp is ENABLED
			...
			add_filter( 'the_content', array( $this, 'the_wri_content_page_top' ), 1200 );
			//Display related in place of WooCommerce related product on product page
			add_action( 'woocommerce_after_single_product_summary', array( $this, 'the_wri_content_page_woocommerce_product' ), 20 );
			...
			//switch existing taxonomies on different post edit pages
			add_action('init', array( $this, 'taxonomy_for_objet_types' ));
			//make new taxonomy specially for WRI relatedness
			if ( 1 == $wri_general_settings['enable_wri_category'] ) {
				add_action('init', array( $this, 'register_wri_category' ));
			}
			add_action( 'admin_head', array($this, 'set_css_category_column_width_on_list_tables') );
	...
	public $template_url;
	function the_wri_content_page_top($content) {

		if ( in_the_loop() ) {
			remove_filter( 'the_content', array( $this, 'the_wri_content_page_top' ), 1200 );
			remove_filter( 'the_content', array( $this, 'the_wri_content_page_bottom' ), 1200 );
			remove_action( 'woocommerce_after_single_product_summary', array( $this, 'the_wri_content_page_woocommerce_product' ), 20 );
		
			$content = $this->wri_display_related('on_page', 'top', null, TRUE)
				. $this->promote_text()
	...
	function the_wri_content_page_woocommerce_product() {
		...
		if ( in_the_loop() ) {
			...
			$content =
				apply_filters('wri_content_clear_start','<div class="wri_content_clear_both">')
				. apply_filters('wri_content_clear_end','</div>')
				. $this->wri_display_related('on_page', 'wc_product', null, TRUE)
				. $this->promote_text();				
					...
					foreach ($reference2related_option_array as $reference2related_option) {
						...
						if (
							...
							$option_array=array(
									...
									'wri_thumbnail_columns_number' => nvl($related_option['thumbnail_columns_number'], 4),
	...
	function taxonomy_for_objet_types() {
		...
		if (is_array($wri_general_settings['wri_used_posttypes'])) {
			...
			foreach ($wri_used_posttypes as $wri_used_posttype) {
				...
				$related_option=get_option( 'wri_related_items___' . $wri_used_posttype );

				if ( isset($related_option['cross_taxonomies']) and is_array($related_option['cross_taxonomies']) ) {
					foreach ( $related_option['cross_taxonomies'] as $cross_taxonomy => $value ) {

						if ( 1 == $value ) {

						//register_taxonomy_for_object_type( $taxonomy, $object_type );
						register_taxonomy_for_object_type($cross_taxonomy, $related_option['posttype']);
	...
	function register_wri_category() {
		// Add new taxonomy
		$labels = array(
			'name'              => _x( 'WRI Similarity Marker Categories', 'taxonomy general name','wri' ),
			'singular_name'     => _x( 'WRI Similarity Marker Category', 'taxonomy singular name','wri' ),
			'search_items'      => __( 'Search WRI Categories','wri' ),
			'all_items'         => __( 'All WRI Categories','wri' ),
			'parent_item'       => __( 'Parent WRI Category','wri' ),
			'parent_item_colon' => __( 'Parent WRI Category','wri' ).':',
			'edit_item'         => __( 'Edit WRI Category','wri' ),
			'update_item'       => __( 'Update WRI Category','wri' ),
			'add_new_item'      => __( 'Add New WRI Category','wri' ),
			'new_item_name'     => __( 'New WRI Category Name','wri' ),
			'menu_name'         => __( 'WRI Similarity Marker Categories','wri' ),
		);
		...
		register_taxonomy( 'wri_category', $this->wri_used_post_types('names'), $args );
	}
...
// Include WooCommerce API Manager; it should be included before plugin collusion examination, because if collusion exists, the API manager client activation hook should be run (otherwise some important option will be missing and 100 - "Invalid Request" will be generated) 
include_once( 'am/class-api-manager-client.php' );
		...
		function wri_show_plugin_conflict_error_notice() {
			...
			foreach ($wri_conflict_plugin_names as $conflict_plugin_name ) {
				...
				<div class="error notice">
				    <p><?php echo '<b>' . __( 'Plugin temporary disabled', 'wri' ) . ':</b> ' . $conflict_plugin_name . '<br>' . __( 'Lite and Pro or old and new versions of the plugin cannot be used at the same time! Deactivate one of them.', 'wri' ) . '<br>' . __( 'If you use an earlier and new version of PRO plugin, please use that one, that <b>name</b> contains the PRO sign. Deatcivate that one, that <b>version</b> number contains PRO.', 'wri' ); ?></p>
				</div>
				<?php			
			} 
		}	
