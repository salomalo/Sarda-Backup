...
fn='cacie_edit_'+type;// Loop through items for current column
 ...
 */function cacie_enable($){$('.ac-button-toggle-edit').addClass('active');if(typeof window.xeditables==='undefined'){cacie_init($)}// enable cacie when init has run once
else{$(window.xeditables).editable('enable')}$(AC.table_id).addClass('cacie-enabled');window.cacie_edit_enabled=1}/*
 * Enable inline editing
 */function cacie_disable($){$('.ac-button-toggle-edit').removeClass('active');$(window.xeditables).editable('disable');window.cacie_edit_enabled=0;// disable click events
...
if('false'===value||false===value){value=''}el.cacie_xeditable({type:'select2',value:value,select2:{width:200,tags:cacie_options_format_editable(options)}},column,item)};/**
	 * Edit type: togglable
	 *
	 * @since 1.0
